...
export class AppComponent implements OnInit, OnDestroy
{
  ...
  private demoTimerSubscribe: Subscription | undefined;
  ...
  init_input_parameters() {
    ...
    if (app_root_element.getAttribute('standalone_mode') === 'true') {
      ...
      this.getSessionKeyService.disable_model_redirect();
  ...
  initAllAttributes (node: any) { // any ???
    if (node.getAttributeNames()) {
      node.getAttributeNames().forEach((item: string) => {
        try {
