...
import { CurrentUser, UserProfile } from '../_models/currentuser';
...
import { ConfigService } from './config.service'; // Circular dependency
...
export class GetSessionKeyService {
  ...
  constructor(
    ...
    protected configService: ConfigService // protected _fuseConfigService: FuseConfigService,
  ) {
  ...
  get_session_key() {
    ...
    try {
      if (
        this.getApiUrlService.get_current_entity().get_app_session_key() != ''
      ) {
  ...
  disable_model_redirect() {
    this.modelRedirect = false;
  }
  enable_model_redirect() {
    this.modelRedirect = true;
  }
      ...
      .subscribe((result: any) => {
        if (result.state === 'success') {
          if (result.data.group_id != null) {
            ...
            this.currentUserProfile.group_id.value_string =
          ...
          if (result.data.user_id != null) {
            ...
            this.currentUserProfile.user_id.value_string =
  ...
  enable_guest_mode(): void {
    console.log(
      'apps.realty.enable_guest_mode ' +
        this.configService.getConfigValue('apps.realty.enable_guest_mode')
    );
    if (
